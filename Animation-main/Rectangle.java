   ...
   public static void main(String[] args) throws IOException {
      ...
      for (int i = 0; i < rows; i++) {
         for (int j = 0; j < cols; j++) {
            ...
            rectWidth = randy.nextDouble() * width;// always smaller than width
            ...
            // do the drawing   
            drawing[i][j].draw();// draws individual rectangle in each column of each row of rectangle
            StdDraw.pause(200);
            outfile.println(drawing[i][j]);  // calls toString
         }
      }
      outfile.flush();// puts buffer (stream + memory) into file
      outfile.close();//closes outfile object
   }
