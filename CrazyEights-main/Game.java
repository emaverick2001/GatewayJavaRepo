...
public class Game {
   ...
   private void initializeGame() {
      ...
      for (int p = 0; p < players.length; p++) {
         int cardsPassed = 0;
         while (cardsPassed < handSize) {
            players[p].getHand().addCard(drawPile.discard());
            cardsPassed++;
         }
      }
   ...
   private Player nextPlayer() {
      ...
      //initializes current player to be at 0
      current++;
  ...
  /**
   ...
   public static boolean cardMatches(Card card1, Card card2) {
      ...
      else if (card1.getRank() == 8) {
         match = true;
      }
      else if (card2.getRank() == 8) {
         match = true;
      }
      return match;
   ...
   private boolean gameOver() {
      ...
      // if current players hand is empty, game is over
      return players[current].getHand().isEmpty(); // Replace this stub
